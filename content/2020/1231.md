...
- 编程
    - static analysis，又发现新玩具了
    - feedbox 迁移到 pnpm
        - 我觉得 pnpm 的做法非常好。如果是编译型的语言，每个 module 写清楚自己依赖的版本，应该可以不依赖目录结构，就会更简单一些。
        - 部分代码给后端使用，需要 CommonJS。前端打包用 rollup，需要 ES6。这种分歧导致代码复用变得麻烦。还是想要一个类似 deno 的打包工具。
        - 迁移后切换了生成代码的目录，忘记更新 scheduler，导致服务停工两天…
    - feedbox 修改了查询链接的方式
        - 一些网站会全文输出 RSS，一次近千条记录
        - 一些新闻网站更新勤快，有上千条记录
        - 这两个查询，需要优化一下
    - 写一些简单的游戏逻辑
        - 斗地主
            - 分排和比大小都很简单
            - 输入一些牌，如何知道牌型？代码写得很傻…
            - 给出上家牌型，如何给出当前手牌的提示？目前是遍历。玩家手牌也就 20 张，算法再差也差不到哪去吧…
        - 四国军棋
        - HOF
- 设备
    - 入手树莓派
        - 买了个插 GPIO 的显示器，然后发现和外壳不太兼容，和风扇也不太兼容。还好对显示器的要求不高…
        - 旧的 TF 转 SD 的转换器不好使，加购一个 TF 读卡器。
        - 不知道是 MBP 问题，还是读卡器问题，或是 TF 卡问题，在 virtualbox 下刷卡多次，也不知道最后成功没。
        - 显示屏白屏，不知道问题在哪。连不上无线网，也不知道到底启动没有。加购网线一根。
        - wpa_supplicant 不知道为什么扫不到信号，换 iwd 没遇到问题。
    - 入手 K380
        - 键盘手感一般，很软。
        - 对比平常用 mbp 的键盘，手靠在触摸板旁边。用外接解盘感觉手飘在空中，不太适应。
- 其他
    - ImprovMX 故障一次。突然来邮件说 mx 记录错误，8 小时后又发邮件说 mx 记录恢复了。
- 娱乐
    - 这个季度，虽然遇到疫情影响，但还是有不错的作品。隐瞒之事、公主连接，都算满意。
    - 又开始完四国军棋了
